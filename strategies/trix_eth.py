...
now = datetime.now()
log_prefix = 'TRIX ETH [' + now.strftime("%d-%m-%Y %H:%M:%S") + ']'
...
logger = BotLogging(
    config.strategies.TrixETH.slack.url,
    config.strategies.TrixETH.slack.username,
    config.strategies.TrixETH.slack.channel
)

ftx = SpotFtx(
    apiKey=config.strategies.TrixETH.apiKey,
    secret=config.strategies.TrixETH.secret,
    subAccountName=config.strategies.TrixETH.subAccountName
)
