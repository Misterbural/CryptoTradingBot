...
now = datetime.now()
log_prefix = 'TRIX BTC [' + now.strftime("%d-%m-%Y %H:%M:%S") + ']'
...
logger = BotLogging(
    config.strategies.TrixBTC.slack.url,
    config.strategies.TrixBTC.slack.username,
    config.strategies.TrixBTC.slack.channel
)

ftx = SpotFtx(
    apiKey=config.strategies.TrixBTC.apiKey,
    secret=config.strategies.TrixBTC.secret,
    subAccountName=config.strategies.TrixBTC.subAccountName
)
