...
now = datetime.now()
log_prefix = 'TRIX ATOM [' + now.strftime("%d-%m-%Y %H:%M:%S") + ']'
...
logger = BotLogging(
    config.strategies.TrixATOM.slack.url,
    config.strategies.TrixATOM.slack.username,
    config.strategies.TrixATOM.slack.channel
)

ftx = SpotFtx(
    apiKey=config.strategies.TrixATOM.apiKey,
    secret=config.strategies.TrixATOM.secret,
    subAccountName=config.strategies.TrixATOM.subAccountName
)
...
if buyCondition(df.iloc[-2]):
    ...
    atomBalance = ftx.get_balance_of_one_coin('ATOM')
    if float(atomBalance) > minTokenForSell:
        sell = ftx.place_market_order(pair, 'sell', atomBalance)
