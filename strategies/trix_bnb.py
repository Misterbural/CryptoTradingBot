...
now = datetime.now()
log_prefix = 'TRIX BNB [' + now.strftime("%d-%m-%Y %H:%M:%S") + ']'
...
logger = BotLogging(
    config.strategies.TrixBNB.slack.url,
    config.strategies.TrixBNB.slack.username,
    config.strategies.TrixBNB.slack.channel
)

ftx = SpotFtx(
    apiKey=config.strategies.TrixBNB.apiKey,
    secret=config.strategies.TrixBNB.secret,
    subAccountName=config.strategies.TrixBNB.subAccountName
)
...
if buyCondition(df.iloc[-2]):
    ...
    bnbBalance = ftx.get_balance_of_one_coin('BNB')
    if float(bnbBalance) > minTokenForSell:
        sell = ftx.place_market_order(pair, 'sell', bnbBalance)
